...
--EX Q
SELECT DISTINCT E.DESIGNACAO FROM ESPECIALIDADE E, MEDICOS M
WHERE M.ESP_ID = E.ESP_ID
AND M.ID_MEDICO NOT IN (
  SELECT M1.ID_MEDICO FROM MEDICOS M1, CONSULTAS C1, PACIENTES P1
  WHERE M1.ID_MEDICO = C1.ID_MEDICO
  AND C1.ID_PACIENTE = P1.ID_PACIENTE
  AND P1.NOME_D = 'Carlos');
  
--EX R
SELECT M.NOME_M FROM MEDICOS M, CONSULTAS C, RECEITAS R, MEDICAMENTOS MD
WHERE R.MEDICAMENTO_ID = MD.MEDICAMENTO_ID
AND M.ID_MEDICO = C.ID_MEDICO
AND R.ID_CONSULTA = C.ID_CONSULTA
GROUP BY M.NOME_M
HAVING COUNT(MD.MEDICAMENTO_ID) > 3;
